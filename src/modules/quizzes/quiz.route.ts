...
import {
  ...
  quizIdParamValidationSchema,
  submitQuizValidationSchema,
  updateQuizValidationSchema
} from './quiz.validation';

const router = Router();

router.get('/:quizId/public', validateRequest(quizIdParamValidationSchema), QuizController.getPublicQuiz);
router.post(
  '/:quizId/submit',
  optionalAuthenticate,
  validateRequest(submitQuizValidationSchema),
  QuizController.submitPublicQuiz
);
